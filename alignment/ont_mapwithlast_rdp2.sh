...
if [ $# -lt 5 ] ; then
    echo "Usage: ont_mapwithlast_rdp.sh readsfastq readsfmt readgroup /path/to/reference.fasta outdir a m lastal_exe"
    ...
    echo "           lastal_exe should contain path to lastal (if standard install using make install, simply set this to lastal)"
    echo
    exit 1
fi
...
if [ ! -f ${outdir}/${reads_basename}.last.txt ] ; then
    ...
    echo "${lastal_exe} -m ${lastal_m} -Q 0 -r 1 -q 1 -a ${lastal_a} -b 1 -s 2 ${ref_dirname}/${ref_basename}.lastindex ${readspath} > ${outdir}/${reads_basename}.last.txt"
          ${lastal_exe} -m ${lastal_m} -Q 0 -r 1 -q 1 -a ${lastal_a} -b 1 -s 2 ${ref_dirname}/${ref_basename}.lastindex ${readspath} > ${outdir}/${reads_basename}.last.txt
    elif [ $readsfmt = 'fastq' ] ; then
    echo "${lastal_exe} -m ${lastal_m} -Q 1 -r 1 -q 1 -a ${lastal_a} -b 1 -s 2 ${ref_dirname}/${ref_basename}.lastindex ${readspath} > ${outdir}/${reads_basename}.last.txt"
          ${lastal_exe} -m ${lastal_m} -Q 1 -r 1 -q 1 -a ${lastal_a} -b 1 -s 2 ${ref_dirname}/${ref_basename}.lastindex ${readspath} > ${outdir}/${reads_basename}.last.txt
    else
